...
| 2026-02-20 | Corregir acoplamiento de adapters a handlers concretos | En ReservationService y Producer, los adapters de entrada (RabbitMQConsumer/API Controllers) usaban directamente clases handler en lugar de puertos de entrada. Se introdujeron interfaces de caso de uso (`IProcessReservationUseCase`, `IReserveTicketUseCase`, `IRequestPaymentUseCase`) y se actualizo DI para resolver por interfaz. Tambien se eliminaron abstracciones/codigo huérfano (`IMessageConsumer`, `TicketNotAvailableException`) | Jorge |
