...
- Siempre evalúa concurrencia en reserva, consistencia eventual e idempotencia cuando el cambio toca flujos distribuidos.
- Si se modifica RabbitMQ, agrega validación de publicación, consumo y compatibilidad de routing keys.
- Si cambia frontend o contrato HTTP, valida que el cliente interpreta correctamente estados y errores.
- Considera `TTL exceeded`, `payment rejected` y liberación del ticket como escenarios obligatorios cuando aplica.
