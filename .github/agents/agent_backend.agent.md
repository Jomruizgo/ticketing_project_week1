...
- Trata `producer` como API de comando asíncrono: no conviertas reserva o pago en respuesta final síncrona si el flujo vigente usa `202 Accepted`.
- Si cambias eventos o routing keys, actualiza también `scripts/setup-rabbitmq.sh`.
- Si cambias el flujo de reserva, preserva el optimistic locking basado en `status` + `version`.
- Si cambias el flujo de pago, preserva validación de TTL, idempotencia y transición consistente de ticket/pago/historial.
- Si cambias contratos consumidos por el frontend, documenta y propaga el ajuste a `frontend/lib/api.ts` y tipos relacionados.
- Por defecto, trata cada HU nueva como modificación del backend existente: primero busca puntos de extensión/cambio en el código real antes de crear implementaciones paralelas.
- Si la HU omite impactos sobre comportamiento previo, debes detectarlos mediante análisis de handlers, repositorios, contratos y tests existentes.
...
PASO 3 → Ejecutar análisis de impacto sobre código, tests y contratos existentes
PASO 4 → Implementar lógica de negocio por HU modificando/exteniendo el backend actual
PASO 5 → Implementar endpoints según contratos del SPEC
PASO 6 → Implementar capa de persistencia y consistencia
PASO 7 → Implementar producción/consumo de eventos (si aplica)
PASO 8 → Implementar taxonomía de errores del dominio
PASO 9 → Instrumentar observabilidad (logs, métricas, trazas)
PASO 10 → Aplicar codificación segura: validación y sanitización
