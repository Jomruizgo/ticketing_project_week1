...
## Restricciones específicas de TicketRush

- Usa `frontend/lib/api.ts` como cliente canónico; no dupliques llamadas HTTP dispersas sin necesidad.
- Modela explícitamente la diferencia entre aceptación del comando (`202`) y estado final confirmado.
- Si cambian estados de ticket o pago, revisa componentes, hooks y badges que interpretan esos estados.
- Si el backend cambia contratos, ajusta tipos y consumidores del frontend en la misma iteración.
- Prioriza UX clara para flujos asíncronos: loading, espera, éxito final, rechazo, expiración o timeout.
- Por defecto, trata cada HU nueva como evolución del frontend existente: primero revisa componentes, hooks, rutas y tipos actuales antes de crear piezas nuevas.
- Si la HU no menciona impactos sobre UI previa, debes detectarlos revisando contratos, estados compartidos y cobertura de regresión existente.
...
PASO 3 → Ejecutar análisis de impacto sobre componentes, hooks, rutas y contratos existentes
PASO 4 → Implementar componentes reutilizables según design system
PASO 5 → Conectar APIs según contratos del SPEC (con manejo de errores y estados de carga)
PASO 6 → Gestionar estado con el patrón definido en arquitectura
PASO 7 → Implementar navegación, rutas y carga bajo demanda
PASO 8 → Implementar validaciones de formularios
PASO 9 → Implementar autenticación en el cliente
PASO 10 → Instrumentar observabilidad hacia el backend
PASO 11 → Aplicar codificación segura: sanitización, sin secretos en cliente
PASO 12 → Activar skills de código limpio y testing
PASO 13 → Preparar PR documentado con checklist completo
```
