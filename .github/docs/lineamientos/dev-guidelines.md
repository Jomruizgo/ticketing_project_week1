...
---
## 12. Anexo obligatorio para TicketRush

### Contratos e integraciones
- Si cambias un endpoint consumido por el frontend, actualiza también `frontend/lib/api.ts` y sus tipos asociados.
- Si cambias una routing key, cola o evento, actualiza `scripts/setup-rabbitmq.sh` y los servicios consumidores.
- Si cambias el esquema de persistencia o enums, revisa `scripts/schema.sql`, mappings EF/Npgsql y consumidores de estado.

### Evolución de funcionalidades existentes
- Por defecto, una HU nueva se implementa como **delta sobre el sistema actual**, no como desarrollo greenfield.
- Antes de crear archivos o componentes nuevos, inspecciona si ya existe código que implementa parte del flujo y determina si corresponde **extender, corregir o refactorizar**.
- Si la HU no menciona explícitamente impactos colaterales sobre flujos previos, el agente debe realizar análisis de impacto en código, contratos, tests y documentación antes de implementar.
- Está prohibido duplicar lógica o crear una segunda ruta funcional paralela cuando el cambio real consiste en modificar comportamiento existente.
- Si el análisis detecta que la HU altera criterios previos no documentados, el agente debe dejar trazabilidad del impacto y proponer ajustes de regresión.

### Flujos asíncronos
- `producer` responde aceptación, no resultado final de negocio, para reserva y pago.
- El frontend debe modelar estados intermedios y consistencia eventual.
- Los workers deben diferenciar fallos técnicos de validaciones de negocio.

### Reglas críticas del dominio
- No romper el optimistic locking de reserva basado en `status` + `version`.
- No eliminar validaciones de TTL e idempotencia en pagos.
- No duplicar la topología RabbitMQ fuera de `scripts/setup-rabbitmq.sh`.
