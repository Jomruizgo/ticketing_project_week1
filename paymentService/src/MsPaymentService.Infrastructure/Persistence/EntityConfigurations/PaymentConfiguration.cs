...
public class PaymentConfiguration : IEntityTypeConfiguration<Payment>
{
    public void Configure(EntityTypeBuilder<Payment> builder)
    {
        ...
        builder.Property(p => p.TicketId).HasColumnName("ticket_id");
        builder.Property(p => p.Status).HasColumnName("status").IsRequired();
        builder.Property(p => p.ProviderRef).HasColumnName("provider_ref").HasMaxLength(120);
        ...
        builder.HasOne(p => p.Ticket).WithMany(t => t.Payments).HasForeignKey(p => p.TicketId);
        builder.HasIndex(p => p.TicketId).HasDatabaseName("idx_payments_ticket_id");
